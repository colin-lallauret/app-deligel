...
* **Livreur / Chauffeur (`driver`) :** Accède à ses tournées, valide ses passages, consulte les fiches clients, capture les positions GPS et ajoute des notes/photos historiques.
* **Administrateur (`admin`) :** Possède tous les droits du livreur, accède aux données de l'ensemble des tournées, et dispose du droit exclusif de créer/modifier/attribuer les tournées (option activable à distance pour verrouiller l'autonomie actuelle des livreurs).
